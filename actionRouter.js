...
const ActionModz = require('./data/helpers/actionModel.js');
...
router.put('/:id', validateId, validateAction, (req, res) => {
    ...
    ActionModz.update(req.action.id, updatedAction)
        .then(action => {
...
router.delete('/:id', validateId, (req, res) => {
    ActionModz.remove(req.action.id)
        .then(count => {
            res.status(200).json({
...
function validateProjectId(req, res, next){
    ActionModz.get(req.body.project_id)
        .then(project => {
            if (project){
                next();
            } else {
